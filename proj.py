    ...
    #leftmost column excluding 1st and last row
        ...
        for i in range(x-1, x+2):
            for j in range(y, y+2):
                if ((i!=x or j!=y) and mat[i][j]!=0 and ([i, j] not in visited[x][j])):
                    q.append([i, j])
                    visited[i][j].append([x, y])

    #bottom left corner
    elif (x == rows-1 and y == 0):
        if (mat[x][y+1] != 0 and ([x, y+1] not in visited[x][j])):
            q.append([x, y+1])
            visited[x][y+1].append([x, y])
        
        if (mat[x-1][y+1] != 0 and ([x-1, y+1] not in visited[x][j])):
            q.append([x-1, y+1])
            visited[x-1][y+1].append([x, y])

        if (mat[x-1][y] != 0 and ([x-1, y] not in visited[x][j])):
            q.append([x-1, y])
